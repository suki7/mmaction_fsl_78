...
class VideoDataset(BaseDataset):
    ...
    def evaluate(self,
        ...
        # goby
        assert len(results) == len(self.val_or_test), (
            f'The length of results is not equal to the dataset len: '
            f'{len(results)} != {len(self.val_or_test)}')
        ...
        # few-shot val label, goby
        gt_labels =[]
        for items in self.val_or_test:
            labels_per_item =[]
            for item in items:
                labels_per_item.append(item['cls'])
            
            quert_gt = labels_per_item[-1]
            ture_or_false = np.array(labels_per_item[:-1]) == quert_gt
            label = np.repeat(range(self.n_way), self.k_shot)
            label = label[ture_or_false][0]
