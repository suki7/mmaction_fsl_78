...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.goby_video_infos_by_class = self.parse_by_class() # goby

        # get config  goby
        cfg = Config.fromfile("configs/recognition/goby/fewshot_r50_1x1x8_50e_sthv2.py")
        self.n_way = cfg.n_way
        self.k_shot = cfg.k_shot
        self.train_episode = cfg.train_episode
        self.val_episode = cfg.val_episode
        self.training_dataset_class_num = cfg.training_dataset_class_num
        self.test_dataset_class_num = cfg.test_dataset_class_num
    ...
    def prepare_train_frames(self, idx):
        ...
        elif self.goby_video_infos_by_class: #goby
            results = self.goby_video_infos_by_class[idx['cls']][idx['index']]
        else:
    ...
    def __getitem__(self, idx):
        ...
        for id in idx:
            if self.test_mode:
                info = self.prepare_test_frames(id)
                features.append(info['imgs'])
                labels.append(torch.tensor([info['label']]))
            else:
                info = self.prepare_train_frames(id)  
                features.append(info['imgs'])
                labels.append(info['label'])
        result = {}
        # result['label'] = torch.stack(features,dim=0).reshape((-1,features[0].shape[1])+features[0].shape[2:])
        result['imgs'] = torch.cat(features,dim=0)
