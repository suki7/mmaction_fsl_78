...
def main():
    ...
    backbone = osp.join(cfg.work_dir,'backbones',os.path.basename(log_file)[:-4])
    mmcv.mkdir_or_exist(osp.abspath(backbone))
    backbone=backbone.replace(' ', '\ ').replace('(','\(').replace(')','\)').replace('&','\&')
    os.system("cp -r /data1/wrye/mmaction_fsl/mmaction/models/backbones/* %s" % (backbone))
